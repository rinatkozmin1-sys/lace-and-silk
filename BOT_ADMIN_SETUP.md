...
2. В переменных окружения деплоя и в `.env.local` задайте:
...
**Service role** нужен только серверу (Route Handlers): через него бот вставляет и удаляет строки в `products`. На клиент и в репозиторий ключ не выкладывайте.
...
Через `GET /api/products` из таблицы Supabase `products`. Файл `data/products.json` для этого потока не используется.
